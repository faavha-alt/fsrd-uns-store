...
- [x] **Web installer (2026-08-24, commit `b992f95`, sudah di-push)**: wizard instalasi ala WordPress di `/install` (`app/Http/Controllers/InstallController.php`, view `resources/views/install/*`) — cek requirement server, input kredensial DB (tes koneksi via PDO + auto `CREATE DATABASE IF NOT EXISTS`), tulis `.env` (`app/Helpers/EnvHelper.php`), jalankan `key:generate`+`migrate`+`storage:link` lewat `Artisan::call()`, buat akun Admin pertama — semua dari browser, tanpa SSH. Ditandai selesai lewat `storage/installed.lock` (gitignored) yang dicek oleh middleware `EnsureInstalled` (global, di `bootstrap/app.php`) untuk redirect ke `/install` selama lock belum ada, dan mengunci `/install` setelah lock ada. Sebelum lock ada, `AppServiceProvider::register()` memaksa `session`/`cache`/`queue` pakai driver `file`/`sync` (DB belum tentu ada) — lihat detail & pitfall di bawah. `public/index.php` juga otomatis meng-copy `.env.example` → `.env` (dengan `APP_KEY` acak) kalau `.env` belum ada, supaya halaman `/install` bisa tampil sama sekali. **Belum pernah diklik langsung di browser** (PHP lokal dev 8.2, project butuh ^8.3 — tervalidasi lewat `php -l` tiap file + tracing manual siklus request + `composer install --no-dev` sukses beneran di server produksi PHP 8.4 saat bikin master zip di bawah, tapi wizard-nya sendiri belum pernah benar-benar dijalankan sampai selesai di server sungguhan). Coba klik-per-klik dulu di server/lingkungan ber-PHP 8.3+ sebelum dipakai serius untuk pindah server produksi.
  ...
  **Master zip siap pakai untuk instalasi server baru** dibuat dari commit `808dc5b` (bukan dari `fed0f56` yang masih live di produksi) — lihat `scripts/build-release.sh` untuk cara bikin ulang. Lokasi file terakhir: scratchpad sesi Claude Code yang membuatnya (`fsrd-uns-store-808dc5b.zip`, 16MB) — kalau sudah tidak ada, generate ulang dengan script itu dari commit yang sama atau lebih baru. **`Claude.md` dan `tests/` TIDAK ikut** di zip ini (lihat `.gitattributes` — `export-ignore`, ditambahkan 2026-08-24 setelah revisi pertama zip ini sempat tanpa sadar menyertakan `Claude.md` yang isinya detail operasional produksi/SSH/IP server — jangan sampai terulang, selalu cek `unzip -l <zip> | grep -i claude` setelah build kalau ragu; wajar kalau muncul `vendor/guzzlehttp/uri-template/CLAUDE.md` — itu punya paket vendor, bukan punya project).

  **2026-08-27 — fix bug PHP platform requirement drift (commit `808dc5b`)**: install zip lama (`fsrd-uns-store-581a0e4.zip`/`45f0487.zip`) ternyata gagal di server tujuan ber-PHP 8.3.24 dengan error `platform_check.php` minta `PHP >= 8.4.1`. Penyebab: `composer.lock` sempat di-`update` di mesin PHP 8.4 (produksi) tanpa constraint eksplisit, jadi composer bebas naikkan `symfony/clock`/`symfony/css-selector` ke v8.1.x yang butuh PHP 8.4.1+ — padahal `composer.json` menjanjikan `^8.3`. **Fix**: tambah `"config.platform.php": "8.3.99"` di `composer.json` supaya `composer update` ke depan SELALU resolve versi yang kompatibel PHP 8.3 minimum, lalu `composer update` penuh (turun ke `laravel/framework` v13.29.0, `symfony/clock` v7.4.8, dst). Zip baru (`fsrd-uns-store-808dc5b.zip`) sudah diverifikasi `platform_check.php`-nya cuma minta `PHP >= 8.3.0`. **Kalau nanti mau `composer update` lagi, jangan hapus config `platform.php` ini** — itu satu-satunya pengaman supaya lock file tidak drift lagi ke requirement PHP yang lebih tinggi dari yang dijanjikan `composer.json`.
- [~] **Sistem lisensi anti-pindah-tangan — DITUNDA 2026-08-24, kodenya tetap ada tapi TIDAK aktif.** Wizard `/install` sekarang minta license key di step ke-2 (`app/Http/Controllers/InstallController.php::license()/licenseStore()`, view `resources/views/install/license.blade.php`) yang diaktivasi ke **license server terpisah** (lihat di bawah) — key otomatis terkunci ke domain yang aktivasi pertama kali. Setelah terinstall, middleware `EnsureLicensed` (global, di `bootstrap/app.php`, setelah `EnsureInstalled`) mengecek status lisensi secara berkala (throttled, max 1x/jam via `Cache::add`) lewat `app/Helpers/LicenseHelper.php`, disimpan di `Setting::get('license_status')`/`license_last_check_at`. Kalau domain berubah (dipindah tanpa izin) atau di-revoke dari sisi kita → **situs dikunci total** (halaman `resources/views/license-locked.blade.php`, HTTP 503) — untuk gangguan jaringan biasa ada masa tenggang `LICENSE_GRACE_DAYS` (default 3 hari, config `config/license.php`) sebelum ikut terkunci. Ada juga `php artisan license:check` (dipanggil manual/cron) dan `php artisan license:activate {key?}` (retrofit instalasi lama).

  **Kenapa ditunda:** rencana awalnya license server pindah dari `lisensi.mipa.uns.ac.id` (di jaringan kampus UNS, ketahuan diblokir firewall kampus untuk instalasi lain yang juga di jaringan UNS — mis. `jogo-os`/`smart.mipa.uns.ac.id`, yang belakangan ketahuan **satu server fisik yang sama** dengan `lisensi.mipa.uns.ac.id`, jadi sebenarnya soal hairpin NAT bukan cuma firewall kampus) ke `lisensi.favha.cloud` (VPS yang sama dengan `project.favha.cloud`, IP publik `158.140.166.30`, di-Cloudflare-proxy). Migrasi sempat jalan (site `favha-lisensi` sudah dibuat di CloudPanel, DB MySQL `lisensi`/`lisensi` sudah ada di situ) tapi macet di masalah Cloudflare (redirect loop — kombinasi origin cert self-signed + kemungkinan Page Rule/Redirect Rule yang salah + sempat salah kasih IP DNS ke `203.6.149.150` alih-alih `158.140.166.30`) yang belum tuntas dicek. **Belum ada data yang dimigrasi ke `lisensi.favha.cloud`** — data lisensi asli (1 key `fsrd-uns-store` → `project.favha.cloud`) masih cuma ada di MySQL `lisensi.mipa.uns.ac.id`.

  **Perilaku sekarang (2026-08-24, commit `45f0487`)**: `InstallController::license()` **otomatis skip** (bukan hard-block) kalau `LICENSE_SERVER_URL`/`LICENSE_API_SECRET` tidak dikonfigurasi — jadi wizard `/install` jalan normal ke server manapun tanpa perlu license server apa pun. Ini sengaja dibalik dari perilaku hard-block yang sempat dibuat sebelumnya (lihat commit `8344ce8`) — user secara eksplisit minta prioritas "gampang install ke server lain dulu", proyek lisensi ditunda. `scripts/build-release.sh` juga TIDAK di-jalankan dengan `LICENSE_SERVER_URL`/`LICENSE_API_SECRET` untuk sementara — zip yang beredar sekarang (`fsrd-uns-store-45f0487.zip`) murni installer tanpa gate lisensi.

  **Cara mengaktifkan lagi nanti**: (1) selesaikan migrasi `lisensi.favha.cloud` (cek Cloudflare Page Rules/Redirect Rules dulu, pastikan DNS A record `158.140.166.30`, opsional issue Let's Encrypt cert kalau mau Full Strict) ATAU pakai `lisensi.mipa.uns.ac.id` apa adanya untuk client yang tidak di jaringan UNS; (2) migrasikan data key dari MySQL lama ke yang baru kalau pindah host; (3) generate `api_secret` baru untuk instance baru; (4) jalankan `scripts/build-release.sh` dengan `LICENSE_SERVER_URL`/`LICENSE_API_SECRET` di-set; (5) opsional: kembalikan `InstallController::license()` ke perilaku hard-block (lihat riwayat commit `8344ce8` sebelum `45f0487` untuk kode aslinya) kalau memang mau wajibkan lisensi lagi.
